...
class Map
{
    ...
    public function add_marker($params = array())
    {
        ...
        $marker['content'] = '';                        // If not blank, creates an infowindow (aka bubble) with the content provided. Can be plain text or HTML
        ...
        if ($marker['content'] != "") {
            ...
            // Escape any quotes in the event that HTML is being added to the infowindow
            $marker['content'] = str_replace('\"', '"', $marker['content']);
            $marker['content'] = str_replace('"', '\"', $marker['content']);

            $marker_output .= '
            marker_'.$marker_id.'.set("content", "'.$marker['content'].'");';
        }
    ...
    public function create_map()
    {
        ...
        if ($this->zoom == "auto") {
            $this->output_js_contents .= '
            fitMapToBounds_'.$this->map_name.'(lat_longs_map, '.$this->map_name.');
            ';
        }
        ...
        if ($this->places) {
            $this->output_js_contents .= 'function placesCallback(results, status) {
                if (status == google.maps.places.PlacesServiceStatus.OK) {
                    ...
                    ';
            if ($this->zoom == "auto") {
                $this->output_js_contents .= 'fitMapToBounds_'.$this->map_name.'(lat_longs_map, '.$this->map_name.');';
            }
            $this->output_js_contents .= '
                }
            }
        ...
        if ($this->zoom == "auto") {
            $this->output_js_contents .= '
            function fitMapToBounds_'.$this->map_name.'(lat_longs_map, '.$this->map_name.') {
